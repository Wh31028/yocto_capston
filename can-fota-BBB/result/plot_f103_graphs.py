...
# ── CSV 로드 ──────────────────────────────────────────────────
rows = []
with open('/Users/wh31028/repos/capston/can-fota-BBB/fota_results.csv') as f:
...
def plot_graph(field, ylabel, val_fmt, pct_fmt, filename, title):
    ...
    fig.suptitle(title + f"\n({SIZE_LABEL} Firmware, 500kbps CAN, n=30 trials, ±1σ)", fontsize=14, fontweight='bold')
...
plot_graph('time', 'Transfer Time (s)', "{:.1f}s", "+{:.1f}%", "graph1_transfer_time_f103.png", "CAN Firmware Protocol Comparison: Transfer Time vs Packet Loss Rate")
plot_graph('overhead', 'Retransmission Overhead (%)', "{:.2f}%", "", "graph2_overhead_f103.png", "CAN Firmware Protocol Comparison: Retransmission Overhead vs Packet Loss Rate")
plot_graph('total', 'Total Frames (Count)', "{:,.0f}", "+{:.1f}%", "graph3_total_frames_f103.png", "CAN Firmware Protocol Comparison: Total Bus Traffic (TX+RX) vs Loss Rate")

print("\n✅ 모든 F103 그래프(graph1~3) 생성 완료!")
